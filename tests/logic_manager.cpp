...
TEST_SUITE("LogicManager") {
	...
	TEST_CASE("Invalid Entity deletion attempts are ignored") {
		LogicManager logic;
		logic.newEntity(EntityType::planet, "Earth");
		logic.newEntity(EntityType::moon, "Moon");

		CHECK_FALSE(logic.deleteEntity("Venus"));
	}
	...
	TEST_CASE("A mock two-body system can be stored and simulated using LogicManager's clock functions") {
		...
		SUBCASE("Has universe time incremented correctly?") {
			CHECK(logic.getUniverseClock() == 100);
		}

		SUBCASE("Are entities moving?") {
